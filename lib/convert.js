...
function call_func_ignore_exception(func, default_result, args) {
  try {
  ...
  for(var key in template) {
    ...
    if(value instanceof Array) {
      ...
      if(flag) {
        ...
        conv_json[key] = call_func_ignore_exception(conv_func, undefined, values);
      } else {
        conv_json[key] = value;
      }
      continue;
    ...
    if(typeof value == 'function') {
      var conv_func = value;
      conv_json[key] = call_func_ignore_exception(conv_func, undefined, [json]);
      continue;
    }
